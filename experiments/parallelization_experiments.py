...
def run_experiment_in_docker(L, i, project_git_url, target, subcommand, parallelism):
    logging.info(
        f"Running {i} docker run --cpus={parallelism} -e PROJECT_URL={project_git_url} -e TARGET={target} -e SUBCOMMAND={subcommand} -e CMD_ARGS='--jobs=256' cizezsy/bazel_build_test:latest")
    proc = subprocess.run(
        [
            f"docker run --rm --cpus={parallelism} -e PROJECT_URL={project_git_url} -e TARGET={target} -e SUBCOMMAND={subcommand} -e CMD_ARGS='--jobs=256' cizezsy/bazel_build_test:latest"],
        capture_output=True,
        text=True,
        shell=True
    )
